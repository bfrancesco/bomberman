			...
			if (controller.isPaused() && !controller.isMultiplayer()) {
				...
				if (!controller.getGioco().isGameOver()) {
					if (!controller.isMultiplayer()) {
						...
						controller.getGioco().checkBricks();
						controller.sendInfo();
					}
					...
					if (!controller.isMultiplayer()) {
						if (controller.getGioco().results(Settings.PLAYER1) == Gioco.VICTORY) {
							System.out.println("YOU WIN!!! TOTAL POINTS: "
									+ controller.getGioco().getPlayer(Settings.PLAYER1).getPoints() + " !!!");
						} else
						...
						if (res == Gioco.VICTORY) {
							System.out
									.println("YOU WIN!!! TOTAL POINTS: "
											+ controller.getGioco()
													.getPlayer(controller.getClient().getOrderConnection()).getPoints()
											+ " !!!");
